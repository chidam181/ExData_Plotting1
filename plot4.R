...
# Plot as required
par(mfcol = c(2,2))
with(household_table, {plot(DateTime, Global_active_power, type = "n", xlab = "", ylab = "Global active power" , axes = F, frame.plot=TRUE)
        axis(1, at=c(as.numeric(min(household_table$DateTime)), as.numeric(min(household_table$DateTime))+86400, as.numeric(min(household_table$DateTime))+2*86400), labels=c("Thu", "Fri", "Sat"))
        axis(2, yaxs = "r")
        with(household_table, points(DateTime, Global_active_power, type = "l"))
        plot(DateTime, Sub_metering_1, type = "n", xlab = "", ylab = "Energy sub metering" , axes = F, frame.plot=TRUE)
        axis(1, at=c(as.numeric(min(household_table$DateTime)), as.numeric(min(household_table$DateTime))+86400
                     , as.numeric(min(household_table$DateTime))+2*86400), labels=c("Thu", "Fri", "Sat"))
        axis(2, yaxs = "r")
        ...
        plot(DateTime, Voltage, type = "n", xlab = "", ylab = "Voltage" , axes = F, frame.plot=TRUE)
        axis(1, at=c(as.numeric(min(household_table$DateTime)), as.numeric(min(household_table$DateTime))+86400
                     , as.numeric(min(household_table$DateTime))+2*86400), labels=c("Thu", "Fri", "Sat"))
        axis(2, yaxs = "r")
        with(household_table, points(DateTime, Voltage, type = "l"))
        plot(DateTime, Global_reactive_power, type = "n", xlab = "", ylab = "Global reactive power" , axes = F, frame.plot=TRUE)
        axis(1, at=c(as.numeric(min(household_table$DateTime)), as.numeric(min(household_table$DateTime))+86400
                     , as.numeric(min(household_table$DateTime))+2*86400), labels=c("Thu", "Fri", "Sat"))
        axis(2, yaxs = "r")
        with(household_table, points(DateTime, Global_reactive_power, type = "l"))
})
